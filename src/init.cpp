...
bool AppInit2(int argc, char* argv[])
{
    ...
    if (!lock.try_lock())
    {
        ThreadSafeMessageBox(strprintf(_("Cannot obtain a lock on data directory %s.  bitGrow is probably already running."), GetDataDir().string().c_str()), _("bitGrow"), wxOK|wxMODAL);
        return false;
    }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of bitGrow") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart bitGrow to complete") << "\n";
            printf("%s", strErrors.str().c_str());
            ThreadSafeMessageBox(strErrors.str(), _("bitGrow"), wxOK | wxICON_ERROR | wxMODAL);
            return false;
        }
        else
    ...
    if (!fNoListen)
    {
        ...
        {
            ThreadSafeMessageBox(strError, _("bitGrow"), wxOK | wxMODAL);
            return false;
        }
    }
    ...
    if (mapArgs.count("-paytxfee"))
    {
        ...
        {
            ThreadSafeMessageBox(_("Invalid amount for -paytxfee=<amount>"), _("bitGrow"), wxOK | wxMODAL);
            return false;
        }
    ...
    if (mapArgs.count("-reservebalance")) // ppcoin: reserve balance amount
    {
        ...
        {
            ThreadSafeMessageBox(_("Invalid amount for -reservebalance=<amount>"), _("bitGrow"), wxOK | wxMODAL);
            return false;
        }
    }
    ...
    if (!CreateThread(StartNode, NULL))
        ThreadSafeMessageBox(_("Error: CreateThread(StartNode) failed"), _("bitGrow"), wxOK | wxMODAL);
